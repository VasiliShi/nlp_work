...
def main():
    ...
    if args.data_file.endswith('pkl'):
        ...
        with open(args.data_file, 'rb') as f:
            text = pickle.load(f)
            # debug: only use 10% of all c codes
            text = text[:len(text)//10]
    else:
        with codecs.open(args.data_file, 'r') as f: # , encoding=args.encoding
            text = f.read()
    ...
    if args.debug:
        ...
        print(x[0])
        logging.info((str(x[0]), str(batche2string(x[0], vocab_loader.index_vocab_dict).encode('utf-8'))))
        logging.info((str(y[0]), str(batche2string(y[0], vocab_loader.index_vocab_dict).encode('utf-8'))))
