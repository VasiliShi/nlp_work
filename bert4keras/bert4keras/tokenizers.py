...
def load_vocab(dict_path, encoding='utf-8', simplified=False, startswith=None):
    ...
    if simplified:  # 过滤冗余部分token
        ...
        for t in startswith: # 首先将startwith token加入到字典中
            new_token_dict[t] = len(new_token_dict)
            keep_tokens.append(token_dict[t])

        for t, _ in sorted(token_dict.items(), key=lambda s: s[1]):
            if t not in new_token_dict: #逐个遍历vocab中的token
                keep = True
                if len(t) > 1: # 如果长度大于1，如果是标点或者特殊富豪，就舍弃掉
...
class BasicTokenizer(object):
    ...
    def tokenize(self, text, maxlen=None, max_length=None):
        ...
        if maxlen is None and max_length is not None:
            print(
                'From tokenizers.py: The argument d. Please use maxlen instead.'
            )
...
class Tokenizer(BasicTokenizer):
    ...
    def decode(self, ids, tokens=None):
        ...
        text = re.sub(' +', ' ', text)# 多个空格合并称一个
        text = re.sub('\' (re|m|s|t|ve|d|ll) ', '\'\\1 ', text) # 把英文的缩写，i' ll 转化为i'll
